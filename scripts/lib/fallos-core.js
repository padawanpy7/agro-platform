...
// un numero sin significado: `lint` y `check` lo dominan justamente porque funcionan.
...
function parsear(texto) {
  ...
  for (const linea of String(texto || '').split('\n')) {
    ...
    // corrio el loop; si esta sucio, eso es un hallazgo, no ruido a filtrar.
...
function agrupar({ filas = [], rotas = [] } = {}, { minimo = 4, desde = null } = {}) {
  ...
  // El ultimo VERDE de cada modo. Sin esto, un modo que dejo de fallar se ve igual que uno que
  // sigue roto: los dos son "N fallos, ultima el <fecha>". Con esto se puede decir si la tool
  // volvio a andar DESPUES del ultimo fallo, que es la unica evidencia -en el log- de que ya no
  // esta roto. Sale de las mismas filas que ya se leyeron: no cuesta una pasada mas.
  // El modo lleva el exit adentro (`tool X (exit 1)`), asi que el verde de una tool nunca cae en el
  // grupo de su fallo. Se cruza por TOOL + primer argumento colapsado, que es lo que identifica
  // "la misma invocacion" con independencia de como termino.
  const claveDe = (f) => modoDe(f).replace(/ \(exit \d+\)$/, '')
  const okPorClave = new Map()
  for (const f of enRango) {
    if (f.exit !== 0) continue
    const k = claveDe(f)
    if (!okPorClave.has(k) || f.fecha > okPorClave.get(k)) okPorClave.set(k, f.fecha)
  }
  for (const g of grupos.values()) {
    const k = g.modo.replace(/ \(exit \d+\)$/, '')
    if (okPorClave.has(k)) g.ultimoOk = okPorClave.get(k)
  }
  ...
  // del loop. Y `--help` con exit != 0 es su propio hallazgo: pedir ayuda no es un error.
...
function informe(r) {
  ...
  if (r.gaps.length) {
    l.push('')
    l.push('  Modos de falla REPETIDOS que no son un gate (esto es un hueco del loop):')
