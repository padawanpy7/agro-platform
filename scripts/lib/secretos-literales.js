...
const CODIGO_TICKET = /^[A-Z][A-Z0-9]*-[A-Z0-9]+$/
const SLUG = /^[a-z0-9]+(-[a-z0-9]+)+$/

function esIdentificadorLegible(valor) {
  if (UUID.test(valor)) return false
  return CODIGO_TICKET.test(valor) || SLUG.test(valor)
}
...
function marcar(encontrados, ruta, i, clave, bruto) {
  ...
  if (!valor || esTemplate(valor) || esIdentificadorLegible(valor) || valor.length < LARGO_MINIMO) return
