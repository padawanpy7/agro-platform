// ablacion.js - que pieza del loop carga peso y cual es candidata a jubilarse.
//
// Uso: node agro.js ablacion [--json] [--minimo N]
//
...
// ejecuto, porque no habia con que medir. Cada pieza del loop codifica algo que el modelo no
// podia solo, y esos supuestos caducan: sin medicion, el loop solo crece.
//
...
if (argv.includes('--help') || argv.includes('-h')) {
  console.log(`Uso: node ${dispatcher.nombre()} ablacion [--json] [--minimo N]`)
...
if (candidatas.length) {
  ...
  console.log(`    3. esta rota y no mira nada        -> \`node ${dispatcher.nombre()} control-negativo\` lo dice`)
} else {
