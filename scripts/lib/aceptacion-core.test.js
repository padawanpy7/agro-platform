...
const DOC = [
  ...
  '```sh',
  'node agro.js db-compilar sql/PKG_X.sql --verificar',
  '```',
  '',
  '## La cuota en USD sale con la tasa del dia',
  '```sh',
  'node agro.js db-sql --archivo tests/t5.sql',
  '```',
  '',
...
test('parsea titulo + comando, y el manual como manual', () => {
  ...
  assert.equal(p.criterios[0].comando, 'node agro.js db-compilar sql/PKG_X.sql --verificar')
...
test('un comando de varias lineas se conserva entero', () => {
  const p = core.parsear('## Uno\n```sh\nnode agro.js db-sql \\\n  --archivo t.sql\n```\n')
  assert.equal(p.criterios[0].comando, 'node agro.js db-sql \\\n  --archivo t.sql')
})
