...
test('un gate que sale 1 hizo su trabajo; una tool de accion que sale 1 es un gap', () => {
  const texto = Array.from({ length: 5 }, () => fila('lint', 1, 'AGENTS.md'))
    .concat(Array.from({ length: 5 }, () => fila('mi-tool', 1, 'ICC-13')))
    .join('\n')
  const r = core.agrupar(core.parsear(texto))
  assert.equal(r.repetidos.length, 2)
  assert.deepEqual(r.gaps.map((g) => g.tool), ['mi-tool'])
})

test('un modo con pocas repeticiones no es un modo: una vez puede ser cualquier cosa', () => {
  const r = core.agrupar(core.parsear([fila('mi-tool', 1, 'crear'), fila('mi-tool', 1, 'crear')].join('\n')))
...
test('el archivo concreto se colapsa: el modo es la tool sobre ese TIPO de cosa', () => {
  const texto = [
    fila('mi-tool', 1, 'cambios/X/tests/a.test.js'),
    fila('mi-tool', 1, 'cambios/X/tests/b.test.js'),
  ].join('\n')
...
test('el subcomando NO se colapsa: cerrar y iniciar son fallas distintas', () => {
  const texto = [
    fila('otra-tool', 1, 'cerrar'), fila('otra-tool', 1, 'cerrar'),
    fila('otra-tool', 1, 'iniciar'), fila('otra-tool', 1, 'iniciar'),
  ].join('\n')
  const r = core.agrupar(core.parsear(texto), { minimo: 2 })
  assert.equal(r.repetidos.length, 2)
})
...
test('pedir ayuda y salir con != 0 es su propio hallazgo', () => {
  const texto = Array.from({ length: 4 }, () => fila('mi-tool', 2, '--help')).join('\n')
  const r = core.agrupar(core.parsear(texto))
...
test('una tool de accion que falla la mitad de las veces sale aparte de los modos', () => {
  const texto = Array.from({ length: 12 }, (_, i) => fila('mi-tool', i < 9 ? 1 : 0, 'ICC-' + i)).join('\n')
  const r = core.agrupar(core.parsear(texto))
...
test('una tool usada tres veces no se acusa de nada', () => {
  const texto = [fila('mi-tool', 1), fila('mi-tool', 1), fila('mi-tool', 1)].join('\n')
  assert.equal(core.agrupar(core.parsear(texto)).casiSiempreFalla.length, 0)
})

test('--desde acota la ventana', () => {
  const texto = [
    fila('mi-tool', 1, 'crear', '2026-08-01T10:00:00'),
    fila('mi-tool', 1, 'crear', '2026-08-31T10:00:00'),
  ].join('\n')
