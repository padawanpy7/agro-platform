...
const NOMBRE = dispatcher.nombre()

if (args.includes('-h') || args.includes('--help')) {
  console.log(`Uso: node ${NOMBRE} traza [--ticket X] [--sesion UUID] [--json] [--vista arbol|timeline|rebotes]`)
  console.log(`     node ${NOMBRE} traza --archivar [--sesion UUID]`)
  console.log('')
...
function fmtHerramientas(n) {
  ...
  if (n.comandosBf && n.comandosBf.length) return `${n.comandosBf.reduce((a, c) => a + c.veces, 0)} agro.js`
  return '???'
}
