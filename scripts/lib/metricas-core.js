...
//   { tipo: 'tool',   ts, ticket, sesion, tool, id, agente? }
//   { tipo: 'result', ts, ticket, sesion, id, resultado? }
//
// `agente` es el subagent_type cuando la tool es un agente; `resultado` es el texto con que volvio
// la tool, que es lo unico que distingue un agente LANZADO de uno que trabajo (ver ES_LANZAMIENTO).

// La FORMA de un comando de terminal, para poder agruparlos. Medido el 18/08 sobre los
// transcripts del repo: 14.657 comandos DISTINTOS en 14.847 corridas, o sea que repeticion
// literal casi no hay -cada uno trae otra ruta, otro ticket, otro flag-. Lo que si se repite es
// la forma, y es ahi donde se ve que merece ser una tool.
//
// Se saca el `cd <ruta> &&` de entrada (sin eso la familia mas grande del repo era "cd") y el
// `VAR=valor` delante, que es entorno y no el comando. Se corta en el primer separador: lo que
// sigue a un `|` es casi siempre un `head`/`grep` de recorte, no el trabajo.
function familiaDeComando(comando) {
  let c = String(comando || '').replace(/\s+/g, ' ').trim()
  if (!c) return null
  c = c.replace(/^cd\s+\S+\s*(?:&&|;)\s*/i, '')
  while (/^[A-Z_][A-Z0-9_]*=\S+\s+/.test(c)) c = c.replace(/^[A-Z_][A-Z0-9_]*=\S+\s+/, '')
  const seg = c.split(/&&|\|\||;|\|/)[0].trim()
  const t = seg.split(/\s+/).filter(Boolean)
  if (!t.length) return null
  if (/^(node|bash|sh|npx)$/.test(t[0])) {
    // El nombre del dispatcher NO se clava: estaba `bf\.js` y al renombrar el repo el regex dejo
    // de matchear, mandando toda invocacion a la familia generica `node <dispatcher>`. Misma
    // leccion que ya estaba escrita en cierre-core.js. Un .js SIN barras en la raiz es el
    // dispatcher; `node scripts/lib/x.test.js` no entra porque tiene ruta.
    if (t[1] && /^[\w-]+\.js$/.test(t[1])) return `node ${t[1]} ${t[2] || ''}`.trim()
...
// Un agente asincrono contesta enseguida con el acuse del LANZAMIENTO -no con su trabajo, que llega
// despues por notificacion-. Medido el 18/08 sobre los transcripts del repo: 58 llamadas, y las
// asincronas devolvian este texto en ~1,7 s. Contar eso como duracion daba `implementer` con
// mediana de 2,5 s, o sea una tabla que decia que los agentes no cuestan nada.
...
function agregar(eventos) {
  ...
  for (const e of eventos) {
    ...
    if (e.tipo === 'result') {
      ...
      // tool. Sumarlo hacia parecer que un agente cuesta dos segundos.
...
function reporte(agregado, { topTools = 8 } = {}) {
  ...
  lineas.push('')
  lineas.push('Lo regenera `node agro.js metricas` leyendo los transcripts de Claude Code.')
  ...
  if (agentes.size) {
    ...
    lineas.push('  la espera humana adentro: la peor corrida medida daba 65 horas porque la sesion siguio')
    lineas.push('  al dia siguiente. Sirve para comparar entre roles, no como costo de computo.')
    lineas.push('')
  ...
  if (comandos.size) {
    ...
    lineas.push('tool: casi ningun comando se repite palabra por palabra -14.657 distintos en 14.847 corridas- pero la')
    lineas.push('forma se repite todo el tiempo.')
    lineas.push('')
    ...
    lineas.push('adentro: por eso un `cat` promedia 30 s y un `echo` 22 s, que no es lo que tardan. Lo que se')
    lineas.push('puede bajar no es lo que tarda cada comando, es CUANTOS viajes hacen falta -una tool que')
    lineas.push('junta cinco pasos en uno ahorra cuatro viajes, corra rapido o lento-.')
    lineas.push('')
    lineas.push('| Forma del comando | Llamadas | % |')
    lineas.push('|---|--:|--:|')
    for (const [familia, n] of filas.slice(0, 15)) {
      lineas.push(`| \`${familia}\` | ${miles(n)} | ${((n / totalLlamadas) * 100).toFixed(1)}% |`)
    }
    if (filas.length > 15) {
      const resto = filas.slice(15).reduce((a, x) => a + x[1], 0)
      lineas.push(`| *(y ${filas.length - 15} forma/s mas)* | ${miles(resto)} | ${((resto / totalLlamadas) * 100).toFixed(1)}% |`)
    }
    lineas.push('')
  }
...
// loop se usa y cual no. Va en el MISMO reporte que el tiempo y los tokens, para no tener que
...
function resumenUso(usos, desde) {
  ...
  const l = []
  l.push('## Uso de las herramientas del loop')
  l.push('')
