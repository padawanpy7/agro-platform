...
La progresión que las fuentes describen: prompt engineering -> context engineering -> harness
...
| **Goal** | que hay que lograr, **congelado** | `jira/<T>/proposal.md` + `tasks.md` |
| **Verification** | como se sabe que lo hecho esta bien | `node agro.js check` + el verifier |
| **Stopping rule** | cuando esta TERMINADO | `jira/<T>/HECHO_CUANDO.md` (lo corre `node agro.js aceptacion`) |
| **Memory** | que sobrevive a la vuelta | `memory/hechos/` + `jira/<T>/PROGRESO.md` (el del loop, `jira/META/`) |
...
**Lo mide `node agro.js loop`**, que no instrumenta nada nuevo: lee `metrics/tool-runs.log`, que el
...
mirar (ver `node agro.js control-negativo`).
...
- **Oráculo incompleto**: lo que el test no mira, no existe.
- **Reward hacking**: optimizar la medicion. Version típica: podar un documento a las apuradas para
...
Para la misma logica, la orquestación mueve el gasto en tokens ordenes de magnitud. Cuatro
...
- **Un segundo esquema de archivos de planificación.** `jira/<T>/` ya tiene esa forma.
