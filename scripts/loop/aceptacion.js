...
//
// Uso: node agro.js aceptacion [--ticket <CLAVE>] [--json] [--listar] [--todos]
//
...
//     toman `process.cwd()` como raiz, asi que `node agro.js <lo que sea>` solo funciona ahi.
...
function carpetaDe(clave) {
  const base = path.join(RAIZ, cambios.carpeta())
...
if (argv.includes('--todos')) {
  const dir = path.join(RAIZ, cambios.carpeta())
  if (!fs.existsSync(dir)) {
    console.error(`no existe ${cambios.carpeta()}/: nada que barrer (no es un OK)`)
    process.exit(2)
  }
...
if (!fs.existsSync(CARPETA)) {
  console.error(`no existe ${cambios.carpeta()}/${CLAVE}: .la clave esta bien escrita?`)
  process.exit(2)
}
...
if (!fs.existsSync(RUTA)) {
  ...
  console.log('')
  console.log(`Creá ${cambios.carpeta()}/${CLAVE}/${ARCHIVO} con un "## titulo" por criterio y, adentro,`)
...
// El shell con el que corre cada criterio: en Windows se busca un sh de verdad (Git Bash o WSL)
// antes de caer a cmd.exe, porque los criterios se escriben en sintaxis sh. Es el paso 2 de
// HN-ACEPTACION-FALSO-ROJO; los otros cuatro pasos de esa ficha siguen abiertos.
const SHELL = (() => {
...
const DIR_REL = path.posix.join(cambios.carpeta(), CLAVE)
...
for (const c of parseado.criterios) {
  ...
  const r = spawnSync(expandir(c.comando), {
    ...
    // sh: un criterio escrito para sh queda en ROJO SIN HABER CORRIDO (HN-ACEPTACION-FALSO-ROJO).
    shell: SHELL,
    encoding: 'utf8',
