...
// Medido el 18/08 sobre los transcripts: un agente ASINCRONO devuelve su tool_result en ~1,7 s con
// el texto "Async agent launched successfully" -es el acuse del lanzamiento- y el trabajo real
// llega despues, por notificacion. Contar ese resultado como duracion daba una tabla que mentia:
// `implementer` con mediana de 2,5 s. Y al reves, un agente BLOQUEANTE devuelve su informe y su
// duracion es wall-clock, con la espera humana adentro: la peor "corrida" daba 65 horas.
const AGENTES = [
  { tipo: 'tool', ts: '2026-08-18T10:00:00.000Z', ticket: 'LOOP', sesion: 's1', tool: 'Agent', id: 'g1', agente: 'implementer' },
  { tipo: 'result', ts: '2026-08-18T10:00:02.000Z', ticket: 'LOOP', sesion: 's1', id: 'g1',
    resultado: 'Async agent launched successfully. (This tool result is internal metadata ...)' },
  { tipo: 'tool', ts: '2026-08-18T10:01:00.000Z', ticket: 'LOOP', sesion: 's1', tool: 'Agent', id: 'g2', agente: 'verifier' },
  { tipo: 'result', ts: '2026-08-18T10:06:00.000Z', ticket: 'LOOP', sesion: 's1', id: 'g2',
    resultado: 'VEREDICTO: OK. Compila en desmond y ALL_ERRORS vacio.' },
]

test('el acuse de un agente asincrono no cuenta como tiempo de trabajo', () => {
  const d = agregar(AGENTES).LOOP
...
test('el reporte tiene una seccion de agentes que dice que NO se puede medir', () => {
  const md = reporte(agregar(AGENTES))
  assert.match(md, /## Agentes/)
  assert.match(md, /implementer/)
  // La honestidad es parte del reporte: un numero sin su advertencia se lee como medicion.
...
test('las familias se cuentan por LLAMADAS: cada una es un viaje al modelo', () => {
  const eventos = [
    { tipo: 'tool', ts: '2026-08-18T10:00:00.000Z', ticket: 'LOOP', sesion: 's', tool: 'Bash', id: 'c1', comando: 'node agro.js db-sql -e "select 1 from dual"' },
    { tipo: 'result', ts: '2026-08-18T10:00:10.000Z', ticket: 'LOOP', sesion: 's', id: 'c1' },
    { tipo: 'tool', ts: '2026-08-18T10:01:00.000Z', ticket: 'LOOP', sesion: 's', tool: 'Bash', id: 'c2', comando: 'node agro.js db-sql -e "select 2 from dual"' },
    { tipo: 'result', ts: '2026-08-18T10:01:20.000Z', ticket: 'LOOP', sesion: 's', id: 'c2' },
    { tipo: 'tool', ts: '2026-08-18T10:02:00.000Z', ticket: 'LOOP', sesion: 's', tool: 'Bash', id: 'c3', comando: 'git status --short' },
    { tipo: 'result', ts: '2026-08-18T10:02:05.000Z', ticket: 'LOOP', sesion: 's', id: 'c3' },
  ]
  const d = agregar(eventos).LOOP
  assert.strictEqual(d.comandos[0].familia, 'node agro.js db-sql')
