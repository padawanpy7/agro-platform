...
//
// Uso: node agro.js vuelta [--ticket <CLAVE>] [--desde <N>] [--json]
...
// Esto no instrumenta nada nuevo -lee el log que el loop ya escribe en cada corrida- y contesta
...
if (argv.includes('--help') || argv.includes('-h')) {
  console.log('Uso: node agro.js vuelta [--ticket <CLAVE>] [--desde <N>] [--json]')
...
// Aca una tarea es una RAMA, asi que el ticket se deduce de ella cuando no se pasa: pedirlo
// siempre haria que el caso normal -estar parado en la rama del ticket- fuera el mas incomodo.
function ticketDeLaRama() {
  try {
    const r = execFileSync('git', ['branch', '--show-current'], { encoding: 'utf8', cwd: RAIZ }).trim()
    return r && r !== 'main' ? r : null
  } catch { return null }
}

const ticket = valorDe('--ticket') || ticketDeLaRama()
let aceptacionOk = null
if (ticket) {
  const r = spawnSync(process.execPath, [path.join(RAIZ, 'agro.js'), 'aceptacion', '--ticket', ticket],
...
console.log(`==> vuelta: ${ventana.length} corrida(s) ${desde ? `(ultimas ${desde})` : 'desde el ultimo cierre en verde'}`)
