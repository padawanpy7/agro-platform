...
const navegadorCompartido = require('../lib/navegador')

const RAIZ = path.resolve(__dirname, '../..')

function ayuda() {
  console.log(`Uso: node agro.js md-a-pdf <archivo.md> [otro.md ...] [--out <archivo.pdf>] [--horizontal]
...
async function convertir(contexto, md, salida, horizontal) {
...
async function main() {
  ...
  const { browser: navegador, context } = await navegadorCompartido.abrirContexto()
  try {
    for (const md of mds) {
      const salida = out
        ? (path.isAbsolute(out) ? out : path.join(RAIZ, out))
        : md.replace(/\.md$/i, '.pdf')
      await convertir(context, md, salida, horizontal)
