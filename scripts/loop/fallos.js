...
//
// Uso: node agro.js fallos [--desde AAAA-MM-DD] [--dias N] [--minimo N] [--json] [--todos] [--gate]
//
...
for (let i = 0; i < argv.length; i++) {
  ...
  if (!CONOCIDAS.has(a)) {
    console.error(`no conozco la bandera "${a}". Uso: node agro.js fallos [--desde F] [--dias N] [--minimo N] [--json] [--todos] [--gate]`)
    process.exit(2)
  }
...
if (argv.includes('--help') || argv.includes('-h')) {
  console.log('Uso: node agro.js fallos [--desde AAAA-MM-DD] [--dias N] [--minimo N] [--json] [--todos] [--gate]')
  console.log('')
...
// loop de hoy, y mezclarlos hace que un hueco ya tapado siga apareciendo arriba de todo.
